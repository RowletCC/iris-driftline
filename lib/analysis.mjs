const fields = {
  ...
  manager: ['Status'],
  systemUsage: ['AllGlobalReferences', 'GlobalUpdateReferences', 'JournalEntries', 'JournalBlockWrites', 'BlockReads', 'BlockWrites', 'LastUpdate'],
  processes: ['State'],
  audit: ['Enabled'],
  auditEvents: ['EventName', 'Enabled', 'Total', 'Written', 'Lost'],
  journals: ['Size', 'CreationTime', 'Reason']
};

export function projectRows(kind, rows) {
  const keep = fields[kind];
  const input = Array.isArray(rows) ? rows : ['manager', 'systemUsage', 'audit'].includes(kind) && rows ? [rows] : [];
...
export function analyze(snapshot) {
  ...
  return {
    ...
    processCount: snapshot.data.processes?.length ?? null,
    journalCount: snapshot.data.journals?.length ?? null,
    auditEnabled: snapshot.data.audit?.[0]?.Enabled ?? null,
    auditLossCount: snapshot.data.auditEvents?.reduce((total, event) => total + (Number(event.Lost) || 0), 0) ?? null,
