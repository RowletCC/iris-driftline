...
- Reads resource, web application, and role metadata for context. Each endpoint is independent: a `403` on one does not suppress the others.
...
The comparison is deliberately narrow. It does not treat a missing endpoint as a deletion. It can compare only the fields returned to the configured account and the first 500 rows from each list endpoint. A human should verify any change before using the Management Portal to act on it.
...
Live smoke test: against a disposable IRIS 2026.2 Community Edition container, all seven selected SysAdmin endpoints responded; the dashboard rendered 16 tasks, 22 upcoming entries, and no error-coded task history records. These counts describe only that local test instance, not a typical production system.
